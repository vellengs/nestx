...
import { Method, ResponseData, ResponseType, Type } from './';
...
export class MethodGenerator {
    ...
    const parameters = this.node.parameters
      .map(p => {
        try {
          ...
          throw new Error(
            `Error generate parameter method: '${controllerId.text}.${
              methodId.text
            }' argument: ${parameterId.text} ${e}`
          );
        }
      })
  ...
  private processMethodDecorators() {
    ...
    const pathDecorators = getDecorators(this.node, decorator => {
      return [
        'PATH',
        'GET',
        'POST',
    ...
    if (pathDecorators) {
      ...
      let normalPath = normalizePath(pathDecorator.arguments[0]);
  ...
  private getMethodSuccessExamples() {
    ...
    if (exampleDecorators.length > 1) {
      throw new Error(
        `Only one Example decorator allowed in '${
          this.getCurrentLocation
        }' method.`
      );
    }
  ...
  private getDecoratorValues(decoratorName: string) {
    const tagsDecorators = getDecorators(
      this.node,
