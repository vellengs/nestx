...
export class SpecGenerator {
    ...
    Object.keys(this.metadata.referenceTypes).map(typeName => {
      ...
      if (!referenceType.typeName.includes('Generic~')) {
        ...
        if (referenceType.additionalProperties) {
          ...
          ].additionalProperties = this.buildAdditionalProperties(
  ...
  private buildPathMethod(
    ...
    const pathMethod: any = (pathObject[method.method] = this.buildOperation(
      controllerName,
      method
    ));
    ...
    if (
      pathMethod.parameters.filter(
        (p: Swagger.BaseParameter) => p.in === 'body'
      ).length > 1
    ) {
